...
describe "Vurl" do
  ...
  describe "#fetch_metadata" do
    context "with a stubbed 200 OK result" do
      before do
        vurl.stub(get_body: File.read(Rails.root.join("spec", "data", "nytimes_article.html")))
      end
      ...
      it "assigns keywords" do
        vurl.should_receive(:keywords=).
          with('Iraq,Iraq War (2003- ),United States Defense and Military Forces,Terrorism,Bombs and Explosives')
        vurl.fetch_metadata
      end
      it "assigns a description" do
        vurl.should_receive(:description=).
          with('The bombing of a Mosul police headquarters on Friday was the deadliest attack against American soldiers in 13 months.')
        vurl.fetch_metadata
      end
    ...
    context "with a redirected https request" do
      it "sets the title" do
        vurl.title = nil
        vurl.url = "http://github.com"
        vurl.fetch_metadata
        vurl.title.should == "Secure source code hosting and collaborative development - GitHub"
      end
    end
  end
